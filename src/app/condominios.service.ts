...
export class CondominiosService {
  ...
  deletar(condominio: Condominio) : Observable<any>{
    return this.http.delete<any>(Conexao.baseURL+`/condominios/${condominio.codigo}`);
  }
  ...
  getCondominioByCnpj(cnpj: string) : Observable<Condominio>{
    return this.http.get<any>(Conexao.baseURL+`/condominios/cnpj/${cnpj}`);
  }

  getContador(contador: number) : Observable<Condominio>{
    return this.http.get<any>(Conexao.baseURL+`/condominios/contador`);
  }
}
